...
# # Generate map with markers using RBF interpolation limited to the Dublin land area.
# def generate_marker_map(monitors):
...
#     # Instead of a heatmap, add markers with tooltips showing the NO2 info on hover
#     for lat, lon, no2 in zip(lat_samples, lon_samples, interpolated_no2):
#         # Ensure the NO2 value is not negative and format it
#         value = max(0, no2)
#         tooltip_text = f"NO₂: {value:.2f}"
#         folium.Marker(location=[lat, lon], tooltip=tooltip_text).add_to(m)
...
#     map_html = generate_marker_map(monitors) if monitors else "Error fetching monitors"
...
#             <title>Dublin Air Quality Map</title>
#         </head>
#         <body>
#             <h1>Dublin Air Quality Map</h1>
...
# Load Dublin boundary polygon from a GeoJSON file
def load_dublin_polygon_from_file(file_path):
    with open(file_path, "r") as f:
        data = json.load(f)
    return shape(data['features'][0]['geometry'])  # First feature as Dublin boundary

# Generate random points within Dublin polygon
def generate_random_points_in_polygon(polygon, num_points):
...
# Fetch all noise monitors from Sonitus API
def fetch_monitors():
    params = {"username": API_USERNAME, "password": API_PASSWORD}
    response = requests.post(MONITORS_API_URL, data=params)
    return response.json() if response.status_code == 200 else None

# Fetch real-time noise pollution data for a specific monitor
def fetch_monitor_data(monitor_id):
    params = {
        "username": API_USERNAME,
        "password": API_PASSWORD,
...
# Fetch all monitor data in parallel (for performance)
def fetch_all_monitor_data(monitors):
    monitor_data = []
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_monitor = {executor.submit(fetch_monitor_data, monitor['serial_number']): monitor for monitor in monitors}
        for future in as_completed(future_to_monitor):
            ...
            if data and isinstance(data, list):
                for d in data:
                    if isinstance(d, dict) and "laeq" in d:  # Noise pollution levels
                        lat = float(monitor.get("latitude"))
                        lon = float(monitor.get("longitude"))
                        noise_level = float(d["laeq"])  # Equivalent continuous noise level (dB)
                        monitor_data.append((lat, lon, noise_level))
                        break
    return monitor_data

# Generate noise pollution heatmap
def generate_heatmap(monitors):
    map_center = [53.349805, -6.26031]  # Dublin
    m = folium.Map(location=map_center, zoom_start=12)
    ...
    latitudes, longitudes, noise_levels = zip(*monitor_data)
    rbf_interpolator = Rbf(latitudes, longitudes, noise_levels, function='linear')

    # Load Dublin boundary polygon
    dublin_polygon = load_dublin_polygon_from_file(r"app/utils/dublin_boundary.geojson")

    # Generate random points within Dublin boundary
    num_samples = 1000
    random_points = generate_random_points_in_polygon(dublin_polygon, num_samples)
    lat_samples = [pt.y for pt in random_points]
    lon_samples = [pt.x for pt in random_points]

    # Interpolate noise levels
    interpolated_noise = rbf_interpolator(lat_samples, lon_samples)

    heatmap_data = [[lat, lon, max(0, val)] for lat, lon, val in zip(lat_samples, lon_samples, interpolated_noise)]
...
def index():
    ...
    return render_template_string("""
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Dublin Noise Pollution Heatmap</title>
        </head>
        <body>
            <h1>Dublin Noise Pollution Heatmap</h1>
